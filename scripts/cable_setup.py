...
class CableLive:
    ...
    def link_cable_channels(self) -> None:
        ...
        if not self.prim_cylinder_item:
            raise ValueError(f'{self.prim_cylinder_item = }')
        ...
        lx.eval(f'channel.link add {{{self.cable_mesh.id}:{CH_SIDES}}} {{{self.prim_cylinder_item.id}:sides}}')
